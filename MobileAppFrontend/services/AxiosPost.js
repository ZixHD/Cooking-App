...
export const toggleFavorite = async(postId) => {
    try{
        const response = await apiRecipe.put("/favorite/" + postId);
        return response;
    } catch (error){
        console.error("Error trying to favorite a post: ", error);
        throw error;
    }
}
...
export const checkFavorite = async(recipeId) => {
    try{
        const response = await apiRecipe.get("/checkFavorite/" + recipeId)
        return response;
    } catch (error) {
        console.error("Error trying to check if the post is favorite: ", error);
        throw error;
    }
}